...
darkImage: "/images/blogs/docker_dark.webp"
lightImage: "/images/blogs/docker_light.webp"
---
...
- Detach from a container without stopping it: Press `Ctrl-p` followed by `Ctrl-q`.
...
- Use the `docker exec` command to run network diagnostic tools inside a container:
...
- Use the `docker logs` command to check for network-related issues:
...
- **Link containers** (Note: This is a legacy feature; use user-defined networks instead):
...
- **MacVLAN networks**: Create a network that allows containers to appear as physical devices on your network:
