...
author: "Aditya"
published: true
excerpt: "A beginner's guide to understanding Rust macros, their syntax, and how to use them effectively."
darkImage: "/images/blogs/rust-macros_dark.webp"
lightImage: "/images/blogs/rust-macros_light.webp"
---

## Introduction

Rust macros are a powerful tool for metaprogramming, allowing developers to write code that generates other code. This can help reduce boilerplate, implement domain-specific languages (DSLs), and extend the language's capabilities.

### Prerequisites

Before diving into Rust macros, it's essential to have a basic understanding of Rust syntax and concepts, including:
...
> You can get the code of this blog here :

<OgLink url="https://github.com/idityaGE/rust-macros/tree/master/rust_examples" />

---

Let's dive into the fascinating world of Rust macros!
...
- **Rules:** A list of `(Pattern => Expansion);` pairs.
  - **Pattern:** Describes the syntax the macro expects. It consists of [_metavariables_](https://lukaswirth.dev/tlborm/decl-macros/macros-methodical.html#metavariables) and literal Rust syntax.
  - **Expansion:** The code that the macro will generate if the pattern matches.

**Metavariables (Fragment Specifiers):**

These are special variables prefixed with a dollar sign (`$`) and followed by an identifier (e.g., `$name`) and a [_fragment specifier_](https://lukaswirth.dev/tlborm/decl-macros/minutiae/fragment-specifiers.html). The fragment specifier tells the macro how to interpret the input.

Here are the common fragment specifiers:

- **`ident`**: An identifier (e.g., variable name, function name, struct name).
  ```rust
  // title: macros/identifiers.rs
...
Procedural macros _must_ reside in their own crate with the `proc-macro` crate type.
...
```rust
// title: my_proc_macro_crate/src/lib.rs
...
```rust
// title: my_app_crate/src/main.rs
...
```rust
// title: my_derive_crate/src/lib.rs
use proc_macro::TokenStream;
use quote::quote;
...
```rust
// title: my_attribute_crate/src/lib.rs
use proc_macro::TokenStream;
use quote::quote;
...
- **`TokenStream` vs. AST([Abstract Syntax Tree](https://en.wikipedia.org/wiki/Abstract_syntax_tree)):** Procedural macros manipulate `TokenStream`. Use `syn` to parse `TokenStream` into an AST for easier manipulation, and `quote` to generate `TokenStream` from an AST.
...
- **Debugging:** Macros can be hard to debug. You can use tools like [`cargo-expand`](https://github.com/dtolnay/cargo-expand) to see the expanded code of your macros, which is invaluable.
- **Hygiene:** Rust macros are _hygienic_. This means that variables and functions generated by a macro won't accidentally clash with variables or functions in the user's code, and vice-versa. This is a huge benefit over older macro systems in other languages.
...
If you want go deep and learn internals there is good resource [The Little Book of Rust Macros](https://lukaswirth.dev/tlborm/introduction.html).
